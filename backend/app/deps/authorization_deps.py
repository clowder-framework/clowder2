        ...
        if (
            ...
            if listener.access is not None:
                group_q = await GroupDB.find(
                    Or(GroupDB.creator == username, GroupDB.users.email == username),
                ).to_list()
                user_groups = [g.id for g in group_q]

                valid_modificaiton = (
                    (admin and admin_mode)
                    or (listener.creator and listener.creator.email == username)
                    or (listener.access.owner == username)
                    or (username in listener.access.users)
                    or (not set(user_groups).isdisjoint(listener.access.groups))
                )
                if not valid_modificaiton:
                    raise HTTPException(
                        status_code=403,
                        detail=f"User `{username} does not have permission on listener `{listener_id}`",
                    )

            if listener.active is True or (
                listener.creator and listener.creator.email == username
            ):
                return True
            else:
                raise HTTPException(
