...
async def get_role(dataset_id: str, db: MongoClient = Depends(get_db),
                   current_user=Depends(get_current_username)) -> RoleType:
...
class Authorization:
    ...
    async def __call__(self, dataset_id: str, db: MongoClient = Depends(get_db),
