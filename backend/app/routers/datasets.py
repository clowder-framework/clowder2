...
async def get_dataset_files(
    ...
    if (await DatasetDB.get(PydanticObjectId(dataset_id))) is not None:
        ...
        page = Paged(
            metadata=page_metadata,
            data=[
                FileOut(id=item.pop("_id"), **item) for item in files_and_count[0]["data"]
            ],
        )
        return page.dict()
...
async def get_dataset_folders(
    ...
    if (await DatasetDB.get(PydanticObjectId(dataset_id))) is not None:
        ...
        folders_and_count = await FolderDBViewList.find(*query).aggregate(
            [_get_page_query(skip, limit, sort_field="created", ascending=False)],
        ).to_list()
...
async def get_dataset_folders_and_files(
    ...
    if (await DatasetDB.get(PydanticObjectId(dataset_id))) is not None:
        ...
        page = Paged(
            metadata=page_metadata,
            data=[
                FileOut(id=item.pop("_id"), **item) if item.get("object_type") == "file" else FolderOut(
                    id=item.pop("_id"),
                    **item) for item
