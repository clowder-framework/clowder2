...
async def _freeze_folder(folder, new_frozen_dataset_id):
    folder_data = folder.dict()
    folder_data["origin_id"] = folder_data.pop("id")
    ...
    if folder_data["parent_folder"]:
        parent_folder = await FolderDB.get(
            PydanticObjectId(folder_data["parent_folder"])
        )
        frozen_parent_folder = await _freeze_folder(
            parent_folder, new_frozen_dataset_id
        )
