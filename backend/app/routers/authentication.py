...
from app.keycloak_auth import create_user, get_current_user, enable_disable_user
...
async def user_enable(
    ...
    if admin:
        if current_username.email == useremail:
            raise HTTPException(
                status_code=403,
                detail=f"You are currently an admin. Admin cannot enable their own self.",
            )
        else:
            if (user := await UserDB.find_one(UserDB.email == useremail)) is not None:
                try:
...
async def user_enable(
    ...
    if admin:
        if current_username.email == useremail:
            raise HTTPException(
                status_code=403,
                detail=f"You are currently an admin. Admin cannot disable their own self.",
            )
        else:
            if (user := await UserDB.find_one(UserDB.email == useremail)) is not None:
                try:
