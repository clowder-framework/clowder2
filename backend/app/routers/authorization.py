...
from app.models.datasets import (
    ...
    DatasetRoles,
    DatasetOut,
    DatasetStatus, DatasetDBViewList,
)
...
async def set_dataset_group_role(
    ...
    if (dataset := await DatasetDBViewList.find_one(DatasetDBViewList.id == dataset_id)) is not None:
...
async def remove_dataset_user_role(
    ...
    if (dataset := await DatasetDBViewList.find(DatasetDBViewList.id == PydanticObjectId(dataset_id))) is not None:
        if (await UserDB.find_one(UserDB.email == username)) is not None:
            if (
                    auth_db := await AuthorizationDB.find_one(
                        AuthorizationDB.dataset_id == PydanticObjectId(dataset_id),
