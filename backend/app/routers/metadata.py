...
async def save_metadata_definition(
    ...
    if (
            md_def := await MetadataDefinitionDB.find_one(MetadataDefinitionDB.name == definition_in.name)
    ) is not None:
        raise HTTPException(
            status_code=409,
    ...
    new_md_def = await MetadataDefinitionDB(**definition_in.dict(), creator=user).insert()
    found = await MetadataDefinitionDB.find_one(MetadataDefinitionDB.id == new_md_def.inserted_id)
...
async def get_metadata_definition(
    ...
    if name is None:
        ...
        root_query = await MetadataDefinitionDB.find_one(MetadataDefinitionDB.name == name)
